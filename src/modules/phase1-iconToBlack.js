/**
 * Phase 1: Icon Click to Red Screen
 * Handles the initial chaos sequence from icon click until the screen goes red
 * (Previously went to black, now goes to red as first transition)
 */

import { startMouseTrail, stopMouseTrail } from './mouseTrail.js'
import { startDesktopAsciiCamera, stopDesktopAsciiCamera } from './desktopCamera.js'
import { startImageTrash, stopImageTrash } from './imageTrash.js'
import { startMinesweeperGame, stopMinesweeperGame } from './minesweeper.js'
import { startTextExplorer, stopTextExplorer } from './textExplorer.js'

let mouseJitterInterval = null
let extrasContainer = null
let redScreenScheduled = false
let redScreenTimeoutId = null
let sequenceTimeouts = []

// Callback to notify when red screen transition is complete
...
/**
 * Start Phase 1 of the chaos sequence
 */
export async function startPhase1() {
  console.log('🌀 Starting Phase 1: Icon to Black Screen...')
  ...
  const sequence = [
    ...
    { name: 'ASCII Camera', fn: startDesktopAsciiCamera, delay: 2030 },
    { name: 'Image Trash', fn: startImageTrash, delay: 2200 },
    { name: 'Minesweeper', fn: startMinesweeperGame, delay: 2400 },
    { name: 'Text Explorer', fn: startTextExplorer, delay: 3350 },
    { name: 'slowSlime', fn: playSlimeGif, delay: 3000},
    { name: 'Extras Spine', fn: startExtrasSpine, delay: 100 },
  ]
...
function playSlimeGif() {

  const img = document.createElement('img')
  img.src = 'silme-2.gif?' + Date.now()
  img.id = 'phase1-slime-gif'
  img.style.cssText = `
    position: absolute;
    top: 0;
    left: 0;
...
function startPixelatedBackgroundTransition() {
  ...
  function fillPixels() {
    if (!document.getElementById('blue-screen-canvas')) return
...
function spawnExtrasIcon(data, canvasHeight) {
  ...
  // Schedule red screen transition 10s after the first extras icon spawns
  if (!redScreenScheduled) {
    redScreenScheduled = true;
    redScreenTimeoutId = setTimeout(() => {
      // Trigger black pixelated transition
      startBlackPixelatedTransition()
    }, 12000);
  }
...
/**
 * Transition to black background (simple black screen)
 */
...
export function cleanupPhase1() {
  ...
  // Remove slime
  const slime = document.getElementById('phase1-slime-gif')
  if (slime) slime.remove()

  // Remove blue screen
  const blueScreen = document.getElementById('blue-screen-canvas')
  if (blueScreen) blueScreen.remove()
  ...
  // Clear any scheduled red screen
  if (redScreenTimeoutId) {
    clearTimeout(redScreenTimeoutId)
    redScreenTimeoutId = null
    redScreenScheduled = false
  }
}

export { redScreenScheduled, redScreenTimeoutId }
